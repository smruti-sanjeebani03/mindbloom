...
export const AdminUsersPage = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      const userList = toArray(response);
  ...
  const handleToggleStatus = async (id, name, isCurrentlyActive) => {
    try {
      setUpdatingUserId(id);

      const response = await apiService.toggleUserStatus(id, isCurrentlyActive);
      ...
      if (success) {
        addToast("Status Updated", `Updated access status for ${name}`, "info");
      } else {
        addToast(
          "Status Update Failed",
          response?.message || `Could not update ${name}'s access status.`,
          "warning",
        );
      }
    } catch (error) {
  ...
  const getJournalCount = (user) => {
    const count = user?.journalCount ?? user?.journal_count ?? 0;
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <thead>
              <tr className="border-b border-[#EFE6DC] dark:border-[#3D3128] text-[#8C7667] dark:text-[#A8988B] font-semibold">
                ...
                <th className="py-2.5 px-3">Journals Created</th>
            ...
            <tbody className="divide-y divide-[#EFE6DC] dark:divide-[#3D3128]">
              ...
              {isLoading ? (
                ...
                filtered.map((u, index) => {
                  const userName = getName(u);

                  const userEmail = getEmail(u);

                  const status = getStatus(u);

                  const journalCount = getJournalCount(u);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <CozyBadge
                          variant={status === "active" ? "sage" : "autumn"}
                        >
                          {status}
                        </CozyBadge>
                      </td>
